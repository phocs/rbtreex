    ...
    int (*compare)(struct rb_node *lft, struct rb_node *rht))
{
    struct rb_node **link = &(root->rb_node), *parent = NULL;
  	...
  	rb_link_node(new_, parent, link);
  	rb_insert_color(new_, root);
	return true;
}
    ...
    int (*compare)(struct rb_node *lft, struct rb_node *rht))
{
    ...
    bool leftmost = (rb_first_cached(root) == NULL ||
                    (*compare)(new_, rb_first_cached(root)) < 0);

    rb_link_node(new_, parent, link);
    rb_insert_color_cached(new_, root, leftmost);
    return true;
}
